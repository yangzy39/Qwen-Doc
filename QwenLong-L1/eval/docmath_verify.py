...
def process_single_item(item, judge_model):
    ...
    resp = extract_solution(item["response"])[0] if extract_solution(item["response"])[0] else item["response"]
    resp = resp.replace("Therefore, the answer is", "").strip().rstrip(".").strip() if resp else None
    pred = item["pred"] if item["pred"] else resp
...
def main():
    ...
    # print rm results
    rm_results = sum([max(item.get("judge_gpt", None), item.get("judge", None)) for item in new_result_data]) / len(new_result_data)
